...
void getPage() {
	...
	responsePage = create_string();
	string_init(responsePage);
	...
	while (1) {
		...
		do{
			if(curConns < MAXCONNS){
				...
				retval = sslWrite(pState->con, reqBuf, size);
				if(retval < 0){
		...
		for(i = 0; i < n; i++){
			...
			switch(pState->iState){
				case GETTOTALANSWERNUM:
					retval = sslRead(pState);
					if (retval < 0) {
						log_err("sslRead");
						break;
					}
					int preprocess = 1;
					string_assign_cstr(responsePage, pState->responseHtml);
					int total = parseResponse(responsePage, &resultAnswers, parseAnswersJsonData, preprocess);
					if(total == ECOMPLETE || total < 0){
						break;
					}
					pState->iState = GETANSWERS;
					pState->totalAnswers = total;
					pState->iLast = 0;

					/* formal request for data */
					pState->sumTimes = ((pState->totalAnswers + REQUESTSIZE-1)/REQUESTSIZE);
					if(pState->curTimes < pState->sumTimes){
						size = MAXREQSIZE;
//						con = makeConnection(&reqfd);
						prepareAnswersRequest(reqBuf, &size, reqUser, (pState->curTimes)++);
						retval = sslWrite(pState->con, reqBuf, size);
						if(retval < 0){
							log_err("GETTOTALANSWERNUM->write");
							break;
						}
					}
					string_clear(responsePage);
					break;
				case GETANSWERS:
					if(pState->curTimes < pState->sumTimes){
						retval = sslRead(pState);
						...
						debug("times: %d\t%ld\n", pState->curTimes+1, list_size(resultAnswers));
						...
						pState->iState = GETTOTALFOLLOWERNUM;
						pState->curTimes = 0;
						pState->sumTimes = 0;
						pState->iLast = 0;
						size = MAXREQSIZE;
						...
						retval = sslWrite(pState->con, reqBuf, size);
						if(retval < 0){
							log_err("GETTOTALFOLLOWERNUM->write");
							break;
						}
					}
					break;
				case GETTOTALFOLLOWERNUM:
					...
					pState->iState = GETFOLLOWERS;
					pState->totalFollowers = total;
					pState->iLast = 0;

					/* formal request for data */
					pState->sumTimes = ((pState->totalFollowers + REQUESTSIZE-1)/REQUESTSIZE);
					if(pState->curTimes < pState->sumTimes){
						size = MAXREQSIZE;
						prepareFollowersRequest(reqBuf, &size, reqUser, (pState->curTimes)++);
						retval = sslWrite(pState->con, reqBuf, size);
						if(retval < 0){
							log_err("GETTOTALFOLLOWERNUM->write");
							break;
				...
				case GETFOLLOWERS:
					if(pState->curTimes < pState->sumTimes){
						...
						debug("times: %d\t%ld\t%ld\n", pState->curTimes+1, list_size(resultFollowers), list_size(resultFollowers));
						pState->curTimes++;
						//add resultFollowers to unvisitedUser
					}else{
						//remove current user, who has been visited
						pState->iState = GETTOTALANSWERNUM;
						pState->curTimes = 0;
						pState->sumTimes = 0;
						pState->iLast = 0;
